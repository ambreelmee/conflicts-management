...
def get_esr_value(bce_field, esr_institution):
    ...
    if not connections.criterion_key:
        if connections.bloc:
            # when bloc is not null, it means that the result is inside a list
            # check that list is not empty
            if esr_institution[connections.bloc]:
                bloc = esr_institution[connections.bloc][0]
