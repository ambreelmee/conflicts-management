...
import os
import requests
from repositories import SireneInstitutionRepository, SireneSnapshotRepository
...
def compare_esr_without_snapshot(siret, business_name, address_1, address_2, zip_code, city, country, city_code, naf, date_ouverture, tranche_effectif, esr_institution):
    logging.info('in compare without snapshot')
    # 1. create missing snapshot
    SireneSnapshotRepository.create(SireneSnapshotRepository(), siret, business_name, address_1, address_2, zip_code, city, country, city_code, naf, date_ouverture, tranche_effectif)
    logging.info('%s: snapshot created', siret)

    # if institution exists in dataESR, check for conflicts    
    check_for_all_conflict(siret, business_name, address_1, address_2, zip_code, city, country, city_code, naf, date_ouverture, tranche_effectif, esr_institution)
    
        
    

def compare_esr_with_snapshot(siret, business_name, address_1, address_2, zip_code, city, country, city_code, naf, date_ouverture, tranche_effectif, snapshot, esr_institution):

    check_for_all_conflict_with_snapshot(siret, business_name, address_1, address_2, zip_code, city, country, city_code, naf, date_ouverture, tranche_effectif, esr_institution, snapshot)
